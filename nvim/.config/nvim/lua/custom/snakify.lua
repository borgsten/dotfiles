...
function M.snake_to_camel(input)
  ...
  while true do
    ...
    local converted = string.sub(input, 1, underscore - 1) .. string.upper(string.sub(input, underscore + 1, underscore + 1))
...
function M.camel_to_snake(input)
  ...
  if string.find(input, '%u') == nil then
    vim.notify(string.format("Cannot convert '%s' to camelCase, contains no upper case letters", input), vim.log.levels.WARN)
    return nil
  end
  ...
  while true do
    ...
    local converted = string.sub(input, 1, u_case - 1) .. '_' .. string.lower(string.sub(input, u_case, u_case))
